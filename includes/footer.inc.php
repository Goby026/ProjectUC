<hr>


<!-- Footer -->
<footer class="page-footer font-small blue pt-4">

    <!-- Footer Links -->
    <div class="container-fluid text-center text-md-left bg-light">

        <!-- Grid row -->
        <div class="row">

            <!-- Grid column -->
            <div class="col-md-6 mt-md-0 mt-3">

                <!-- Content -->
                <h5 class="text-uppercase">Mesa de partes:</h5>
                <p>Dirección: Av. San Luis cuadra 11 S/N Puerta N° 6 Villa Deportiva Nacional (VIDENA) cruce con la Av. Del Aire, Distrito de San Luis Horario: Lunes a viernes de 8:30 hrs. a 16:30 hrs.</p>

            </div>
            <!-- Grid column -->

            <hr class="clearfix w-100 d-md-none pb-3">

            <!-- Grid column -->
            <div class="col-md-3 mb-md-0 mb-3">

                <!-- Links -->
<!--                <h5 class="text-uppercase">Contenidos</h5>-->
<!---->
<!--                <ul class="list-unstyled">-->
<!--                    <li>-->
<!--                        <a href="./Home.php">Inicio</a>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <a href="./losjuegos.php">Los juegos</a>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <a href="./ComiteOrganizador.php">Comite Organizador</a>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <a href="./ceremonia.php">Ceremonia</a>-->
<!--                    </li>-->
<!--                    <li>-->
<!--                        <a href="./Auspiciadores.php">Auspiciadores</a>-->
<!--                    </li>-->
<!--                </ul>-->

            </div>
            <!-- Grid column -->

            <!-- Grid column -->
            <div class="col-md-3 mb-md-0 mb-3">

                <!-- Links -->
                <h5 class="text-uppercase">Contenidos</h5>

                <ul class="list-unstyled">
                    <li>
                        <a href="./Home.php">Inicio</a>
                    </li>
                    <li>
                        <a href="./losjuegos.php">Los juegos</a>
                    </li>
                    <li>
                        <a href="./ComiteOrganizador.php">Comite Organizador</a>
                    </li>
                    <li>
                        <a href="./ceremonia.php">Ceremonia</a>
                    </li>
                    <li>
                        <a href="./Auspiciadores.php">Auspiciadores</a>
                    </li>
                </ul>

            </div>
            <!-- Grid column -->

        </div>
        <!-- Grid row -->

    </div>
    <!-- Footer Links -->

    <div class="footer-copyright text-center py-3 bg-warning">
        <h5>Siguenos <i class="fab fa-facebook-f"></i> <i class="fab fa-instagram"></i> <i class="fab fa-twitter"></i> <i class="fab fa-youtube"></i> <i class="fab fa-linkedin-in"></i> <i class="fab fa-flickr"></i></h5>
    </div>

    <!-- Copyright -->
    <div class="footer-copyright text-center py-0 bg-warning">© 2019 Copyright:
        <a href="./Home.php"> Juegos Panamericanos</a> <br>
        <img src="./images/ondas.png" class="img-fluid" alt="Responsive image">
    </div>
    <!-- Copyright -->

</footer>
<!-- Footer -->




<!--<footer>-->
<!--    <div class="container-fluid">-->
<!--        <div class="row">-->
<!--            <div class="col-md-8 offset-2">-->
<!--                <ul class="list-group list-group-flush" style="padding: 0 !important;">-->
<!--                    <a href="./Home.php" class="list-group-item">Inicio</a>-->
<!--                    <a href="./losjuegos.php" class="list-group-item">Los juegos</a>-->
<!--                    <a href="./ComiteOrganizador.php" class="list-group-item">Comite Organizador</a>-->
<!--                    <a href="./ceremonia.php" class="list-group-item">Ceremonia</a>-->
<!--                    <a href="./Auspiciadores.php" class="list-group-item">Auspiciadores</a>-->
<!--                </ul>-->
<!--            </div>-->
<!--        </div>-->
<!--        <hr>-->
<!--        <div class="row">-->
<!--            <div class="col-md-10 offset-2">-->
<!--                <h5>Mesa de partes:</h5>-->
<!--                <p>Dirección: Av. San Luis cuadra 11 S/N Puerta N° 6 Villa Deportiva Nacional (VIDENA) cruce con la Av. Del Aire, Distrito de San Luis-->
<!--                    Horario: Lunes a viernes de 8:30 hrs. a 16:30 hrs.</p>-->
<!--            </div>-->
<!--        </div>-->
<!--        <hr>-->
<!--        <div class="row">-->
<!--            <div class="col-md-12">-->
<!--                <center>-->
<!--                    <h5>Siguenos <i class="fab fa-facebook-f"></i> <i class="fab fa-instagram"></i> <i class="fab fa-twitter"></i> <i class="fab fa-youtube"></i> <i class="fab fa-linkedin-in"></i> <i class="fab fa-flickr"></i></h5>-->
<!--                </center>-->
<!--                <center><h4>(c)2019 Juegos Panamericanos</h4>-->
<!--                <img src="./images/ondas.png" class="img-fluid" alt="Responsive image">-->
<!--                </center>-->
<!--            </div>-->
<!--        </div>-->
<!--    </div>-->
<!--</footer>-->

<!--<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>-->
<!--<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>-->

<script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>

<script type='text/javascript' src='./jquery/jquery-migrate-1.2.1.min.js'></script>
<script type='text/javascript' src='./jquery/jquery.plugins.js'></script>
<script type='text/javascript' src='./jquery/mediaelement-and-player.min.js'></script>
<script type='text/javascript' src='./jquery/contador.js'></script>


</body>
</html>
